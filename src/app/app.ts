...
export class App {
  ...
  restaurants = signal<Restaurant[]>([
    ...
    specialty: 'Poulet DG', currentRating: 3 },
    { id: 4, name: 'Saveurs du Wouri', district: 'Bonamoussadi',
    specialty: 'Poisson braisé', currentRating: 4 },
    { id: 5, name: "L'Akwa Gourmand", district: 'Akwa',
    specialty: 'Bobolo et sauce arachide', currentRating: 5 },
...
restoUpdate(updateResto:{idUpdate:number, starUpdate:number,clickNumber:number}){
  this.restaurants.update(
    position => position.map(r =>r.id === updateResto.idUpdate ?{...r, currentRating:updateResto.starUpdate}:r)
  )
  if(updateResto.clickNumber===0 ){
    this.restaurants().map(r=>r.id===updateResto.idUpdate?this.sendInfo.update(()=>{cardNote:this.restoNote}):1)
  
  }
}
sendInfo = signal<{name:string,cardNote:number}>({
  name:this.title(),cardNote:this.restoNote
})
}
